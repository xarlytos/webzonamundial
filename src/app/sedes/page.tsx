...
function EstadioCard({ sede, index }: { sede: typeof SEDES[0]; index: number }) {
...
function EstadioDestacado({ sede, badge, badgeColor, index }: { sede: typeof SEDES[0]; badge: string; badgeColor: string; index: number }) {
  const cardRef = useRef<HTMLAnchorElement>(null);
  const imageRef = useRef<HTMLImageElement>(null);
  const imageUrl = STADIUM_IMAGES[sede.slug];
    ...
    <div ref={containerRef} style={{ background: BG, minHeight: '100vh' }}>
      ...
      <section ref={usaRef} className="usa-section max-w-6xl mx-auto px-4 mb-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {usa.map((sede, index) => (
            <EstadioCard key={sede.slug} sede={sede} index={index} />
          ))}
        </div>
      </section>
